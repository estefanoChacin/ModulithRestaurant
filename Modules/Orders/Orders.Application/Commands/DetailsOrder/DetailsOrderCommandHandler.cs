...
{
    public class DetailsOrderCommandHandler(IOrderRepository orderRepository, IMediator mediator, ILogger<DetailsOrderCommandHandler> logger) : IRequestHandler<DetailsOrderCommand, DetailsOrderDto>
    {
        ...
        public async Task<DetailsOrderDto> Handle(DetailsOrderCommand request, CancellationToken cancellationToken)
        {
            ...
            var order = await _orderRepository.GetByIdAsync(request.IdOrder) ?? throw new KeyNotFoundException("Order not found");
            var idsProducts = order.Products.Select(p => p.Id).ToList();

            var listProductss = await _mediator.Send(new GetProductsByIdsQuery(idsProducts), cancellationToken);
            var customer = await _mediator.Send(new GetCustomerByIdQuery(order.IdCustomer), cancellationToken);
